...
export default function AdminPerfumesPage() {
    ...
    const carregarCatalogo = async (nome = "") => {
        ...
        try {
            ...
            // BLINDAGEM: Garante que se não for array, vira uma lista vazia []
            const dadosBrutos = response.data?.content ?? response.data;
            setPerfumes(Array.isArray(dadosBrutos) ? dadosBrutos : []);
        } catch (error) {
            console.error("Erro ao carregar admin:", error);
            setPerfumes([]); // Garante array vazio se a requisição falhar
        } finally {
            setLoading(false);
        }
    };
